function [enhancedImage] = redColor(img)
    format longg;
    format compact;
    fontSize = 13;
    ...
    %enhancement using adaptive histogram
    newEnhancedImage = adapthisteq(redChannelC);
    

    %im_RedHist = histeq(redChannelret);
    [pixelCount grayLevels] = imhist(newEnhancedImage);
    subplot(2, 4, 6); 
    bar(pixelCount);
    grid on;
    ...
    bar(pixelCount);
    grid on;
    caption = sprintf('Enhnd Green Hist');
    title(caption, 'FontSize', fontSize);
    xlim([0 grayLevels(end)]); % Scale x axis manually.
    ...
    bar(pixelCount);
    grid on;
    caption = sprintf('Ehncd Blue Hist');
    title(caption, 'FontSize', fontSize);
    xlim([0 grayLevels(end)]); % Scale x axis manually.
    ...
    set(gcf,'Color','[0.12,0.34,0.19]');
    subplot(2, 1, 1)
    imshowpair(redChannelC, newEnhancedImage, 'montage')
    title('Red Enhanced Output')
    enhancedImage = newEnhancedImage;
end