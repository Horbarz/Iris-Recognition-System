close all;
clear all;clc;
PathName = 'C:\Users\ONI\Desktop\LatestIrisRecog\IRIS_CL\black_iris_images\all_image\test_iris\';

for iris = 11:12;
    img_filename = strcat(PathName,num2str(iris),'.jpg')
    ...
    imageswithcircles = uint8(img);

    %get pixel coords for circle around iris
    [x,y] = circlecoords([circleiris(2),circleiris(1)],circleiris(3),size(img));
    % [x,y] = circlecoords([a(2),a(1)],a(3), size(img));
    ind2 = sub2ind(size(img2),double(y),double(x)); 

    %get pixel coords for circle around pupil
    [xp,yp] = circlecoords([circlepupil(2),circlepupil(1)],circlepupil(3),size(img));
    ...
    cd('C:\Users\ONI\Desktop\LatestIrisRecog\IRIS_CL\black_iris_images\all_image\test_iris\templates\')
    imwrite(template, strcat('a',num2str(iris),'.jpg'),'jpg');
    imwrite(mask,strcat('b',num2str(iris),'.jpg'),'jpg');
    template1 = imread(strcat('a',num2str(iris),'.jpg'));
    mask1 = imread(strcat('b',num2str(iris),'.jpg'));
end
